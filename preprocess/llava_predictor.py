...
weights = [{"dest": "liuhaotian/llava-v1.5-13b", # git commit hash from huggingface
    "src": "llava-v1.5-13b/006818fc465ebda4c003c0998674d9141d8d95f8",
    "files": ["config.json", "generation_config.json", "pytorch_model-00001-of-00003.bin",
        "pytorch_model-00002-of-00003.bin", "pytorch_model-00003-of-00003.bin", "pytorch_model.bin.index.json",
...
def download_weights(baseurl: str, basedest: str, files):
...
class Predictor(BasePredictor):
    ...
    def predict(self, image: Path = Input(description="Input image"),
            prompt: str = Input(description="Prompt to use for text generation"), top_p: float = Input(
                ...
                ge=0.0, le=1.0, default=1.0), temperature: float = Input(
        ...
        with torch.inference_mode():
            output_ids = self.model.generate(input_ids, images=image_tensor,
                do_sample=True if temperature > 0 else False, temperature=temperature, top_p=top_p,
...
if __name__ == "__main__":
    prompt="provide a COMPACT and OBJECTIVE caption for the action currently performed by the person in the scene"
    image_file = "frame.jpg"
    
    pred = Predictor()
    pred.setup()
    pred.predict(image_file, prompt, 0.7, 0.2, 512)
    time.sleep(10)
